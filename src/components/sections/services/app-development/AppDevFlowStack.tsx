...
import type { AppDevWorkflowStep } from "@/types/app-development-service";

function AppDevFlowCardItem({
  item,
  index,
  total,
  progress,
}: {
  item: AppDevWorkflowStep;
  index: number;
  total: number;
    ...
    >
      <div className="flex h-screen w-full items-center justify-center bg-[#f7f7f5]">
        <div className="grid h-[calc(100vh-2rem)] w-full grid-cols-1 overflow-hidden border border-black/10 bg-white shadow-[0_20px_60px_rgba(0,0,0,0.08)] lg:grid-cols-[1.05fr_0.95fr]">
          <div className="relative h-[50vh] min-h-80 lg:h-full">
            <Image
              src={item.image}
              alt={item.alt}
              fill
...
export default function AppDevFlowStack() {
  const containerRef = useRef<HTMLElement | null>(null);
  const { steps } = appDevelopmentServiceData;
