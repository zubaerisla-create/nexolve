...
import { aiIntegrationServiceData } from "@/data/ai-integration-service";
import type { AiIntegrationWorkflowStep } from "@/types/ai-integration-service";

function AiFlowCardItem({
  item,
  index,
  total,
  progress,
}: {
  item: AiIntegrationWorkflowStep;
  index: number;
  total: number;
          ...
          <div className="relative h-[50vh] min-h-[320px] lg:h-full">
            ...
            <div className="absolute left-4 top-4 flex items-center gap-3 md:left-6 md:top-6">
              ...
              <span className="text-sm font-medium text-white">
                {String(item.id).padStart(2, "0")} / {String(total).padStart(2, "0")}
              </span>
            </div>
          </div>
...
export default function AiIntegrationFlowStack() {
  ...
  const cards = aiIntegrationServiceData.steps;
